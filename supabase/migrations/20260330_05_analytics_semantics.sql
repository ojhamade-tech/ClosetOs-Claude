-- ─────────────────────────────────────────────────────────────────────────────
-- item_insights_rpc Update (05: Analytics Semantics)
-- Restores backward compatibility of 'worn_count' while explicitly isolating 
-- 'worn_count_global' and 'last_worn_date_global' for strict semantic use.
-- ─────────────────────────────────────────────────────────────────────────────
...
returns table (
  ...
  worn_count             bigint,     -- Backward compatible: Worn in selected period
  worn_count_global      bigint,     -- All-time global explicit
  last_worn_date_global  date        -- All-time global maximum
)
language sql
stable
security definer
...
as $$
  select
    ...
    wi.purchase_price,
    wi.created_at,
    count(wp.id) filter (where wp.date >= start_date and wp.date <= end_date) as worn_count,
